...
def fill(view, data):
    replace_data(view, "Design Template", data["product_name"])
    replace_data(view, "탄력증가로 처짐없는피부!", data["real_review_sentence"].split("\\n")[0])
    replace_data(view, "템플릿 디자인 고객님들의 리얼후기를만나보세요)", data["real_review_sentence"].split("\\n")[1])
